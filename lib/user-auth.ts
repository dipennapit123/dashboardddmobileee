 ...
 * If JWT verification is not configured or fails, falls back to x-clerk-user-id
 * so the mobile app works when CLERK_JWT_ISSUER is missing or mismatched on Vercel.
 */
export async function getUserIdFromRequest(): Promise<string> {
  ...
  if (authHeader?.startsWith("Bearer ") && env.clerkJwtIssuer) {
    try {
      ...
      // JWT verification failed (wrong issuer, JWKS error, etc.). Fall back to header so mobile still works.
